...
class FairseqTask(object):
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False,
        ...
        """
        model.train()
        loss, sample_size, logging_output = criterion(model, sample, gs_tau=gs_tau, gs_hard=gs_hard)
        if ignore_grad:
            loss *= 0
        optimizer.backward(loss)
        return loss, sample_size, logging_output

    def valid_step(self, sample, model, criterion, gs_tau=0.5, gs_hard=False):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, sample, gs_tau=gs_tau, gs_hard=gs_hard)
        return loss, sample_size, logging_output

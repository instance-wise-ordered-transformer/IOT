...
from fairseq.optim import lr_scheduler, gumbel_softmax_scheduler
...
class Trainer(object):
    ...
    def init_meters(self, args):
        ...
        self.meters['train_kl_loss'] = AverageMeter()
        self.meters['train_old_loss'] = AverageMeter()
        self.meters['valid_loss'] = AverageMeter()
        self.meters['valid_nll_loss'] = AverageMeter()
        self.meters['valid_kl_loss'] = AverageMeter()
        self.meters['valid_old_loss'] = AverageMeter()
    ...
    @property
    def gs_scheduler(self):
        if self._gs_scheduler is None:
            self._gs_scheduler = gumbel_softmax_scheduler.build_gs_scheduler(self.args)
        return self._gs_scheduler
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            try:
                self.get_model().load_state_dict(state['model'], strict=True if not warmup_from_nmt else False)
            except Exception:
                raise Exception(
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                loss, sample_size, logging_output = self.task.train_step(
                    ...
                    ignore_grad, gs_tau=self.get_gs_tau(), gs_hard=False
                )
        ...
        try:
            ...
            self.meters['train_old_loss'].update(logging_output.get('old_loss', 0), sample_size)
            ...
            if 'nll_loss' in logging_output:
                ...
                self.meters['train_kl_loss'].update(logging_output.get('kl_loss', 0), ntokens)
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion,  gs_tau=self.get_gs_tau(), gs_hard=True
                )
            except RuntimeError as e:
        ...
        self.meters['valid_old_loss'].update(logging_output.get('old_loss', 0), sample_size)
        ...
        if 'nll_loss' in logging_output:
            ...
            self.meters['valid_kl_loss'].update(logging_output.get('kl_loss', 0), ntokens)
    ...
    def gs_step_update(self):
        return self.gs_scheduler.step_update(self.get_num_updates())
    ...
    def get_gs_tau(self):
        return  self.gs_scheduler.get_gs_tau()

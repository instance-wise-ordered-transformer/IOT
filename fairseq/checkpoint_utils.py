...
def load_checkpoint(args, trainer):
    ...
    if args.warmup_from_nmt:
        if os.path.isabs(args.warmupfile):
            checkpoint_path = args.warmupfile
        else:
            checkpoint_path = os.path.join(args.save_dir, args.warmupfile)
        assert os.path.exists(checkpoint_path), 'please make sure there is a warmup nmt file'
        print('warmup from {}...'.format(checkpoint_path))
    else:
...
def load_model_ensemble(filenames, arg_overrides=None, task=None, numckts=1):
    ...
    for filename in filenames:
        ...
        decoder_orders = args.decoder_orders
        assert len(decoder_orders) == numckts
        for i in decoder_orders:
            encoder_perm_order = 0
            decoder_perm_order = i
            args.encoder_perm_order = encoder_perm_order
            args.decoder_perm_order = decoder_perm_order

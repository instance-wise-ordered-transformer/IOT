...
class SequenceGenerator(object):
    def __init__(
        ...
        self.soft_ensemble = args.soft_ensemble
        ...
        **kwargs
        ...
        Args:
            ...
            prefix_tokens (torch.LongTensor, optional): force decoder to begin
                with these tokens
        """
        model = EnsembleModel(models, soft_ensemble=self.soft_ensemble)
...
class EnsembleModel(torch.nn.Module):
    ...
    def __init__(self, models, soft_ensemble):
        super().__init__()
    ...
    def forward_decoder(self, tokens, encoder_outs, temperature=1.):
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, weight, attn = self._decode_one(
                tokens,
                model,
                encoder_out,
                self.incremental_states,
        ...
        # weight = torch.mean(torch.stack(weights, dim=0), dim=0)
        weight = weights[0]
        log_probs = torch.stack(log_probs, dim=1)
        avg_probs = torch.bmm(weight, log_probs).squeeze()
    ...
    def _decode_one(
        ...
        if attn is not None:
            if type(attn) is dict:
                attn = attn['attn']
            attn = attn[:, -1, :]
        probs, onehot, soft, _ = model.get_normalized_probs(decoder_out, log_probs=log_probs, gs_tau=0.1, gs_hard=True)
        weight = soft if self.soft_ensemble else onehot
        weight = weight.unsqueeze(1)
        probs = probs[:, -1, :]
        return probs, weight, attn
